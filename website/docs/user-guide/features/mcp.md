...
An authorization server can also advertise two shapes of its own identifier: protected-resource metadata naming a path-scoped server (`https://host/issuer`) whose metadata document answers with the bare origin (`https://host`) — Strava's MCP connector publishes exactly that pair. RFC 8414 §3.3's identity check compares the two, so Hermes treats them as the same authorization server (same scheme, host and port) and accepts the document the advertisement resolves to, instead of parking the connection on an issuer mismatch. A document naming a *different* origin is still rejected, so the check that stops one authorization server from speaking for another is unchanged.
