...
Speech-to-text supports eight providers: local faster-whisper (free, runs on-device — the first use downloads the model, see [Local STT model download](/user-guide/features/voice-mode#local-stt-model-download)), a local command wrapper, Groq, OpenAI Whisper API, Mistral, xAI, ElevenLabs Scribe, and DeepInfra. Voice message transcription works across Telegram, Discord, WhatsApp, and other messaging platforms. See [Voice & TTS](/user-guide/features/tts) and [Voice Mode](/user-guide/features/voice-mode) for details.
