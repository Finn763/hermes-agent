...
def test_checkout_publishes_the_borrower_count_under_the_client_lock():
    """The checkout holds the client lock across BOTH resolving and incrementing.

    ``_ensure_primary_openai_client`` takes/releases that same re-entrant lock internally, so without
    one outer hold a teardown can land between its return and the count going up, read
    ``in_flight == 0`` and hard-close the transport this borrower is about to drive (#107475).
    """
    agent = _make_agent()
    agent.client = _SharedClient()
    lock = agent._openai_client_lock()
    held = []
    ensure = agent._ensure_primary_openai_client

    def _observed(*, reason):
        held.append(lock._is_owned())
        return ensure(reason=reason)

    agent._ensure_primary_openai_client = _observed
    with agent._shared_client_checkout(reason="test_request"):
        pass

    assert held == [True], "the client lock must be held while the borrower count is published"
    assert agent._shared_in_flight() == 0
