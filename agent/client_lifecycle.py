...
class ClientLifecycleMixin:
    ...
    def _acquire_shared_client(self, *, reason: str) -> Any:
        ...
        Resolving/creating the client and publishing the borrower count happen under ONE hold of the
        client lock: ``_ensure_primary_openai_client`` takes/releases that same (re-entrant) lock
        internally, so a teardown landing in the gap would read ``in_flight == 0`` and hard-close the
        transport this caller is about to use (#107475).
        """
        with self._openai_client_lock():
