...
async def _device_metadata(client, server_url, auth_server_url):
    ...
    from tools.mcp_oauth_provider import issuer_identifiers_match
    ...
    for url in build_oauth_authorization_server_metadata_discovery_urls(auth_server_url, server_url):
        ...
        if auth_server_url:
            # A path-scoped advertised identifier whose metadata names the same server's origin is the same
            # authorization server (RFC 8414 §3.3 identity, see ``issuer_identifiers_match``, #116233): bind
            # the comparison to the identifier the metadata itself claims, then let the SDK's own check run.
            claimed = str(metadata.issuer)
            validate_metadata_issuer(
                metadata, claimed if issuer_identifiers_match(claimed, auth_server_url) else auth_server_url)
...
async def _register(client, provider, cfg):
    ...
    # SEP-2352: bind these credentials to the identifier the runtime flow compares them against — the
    # advertised authorization server when the resource advertised one (the SDK's own rule), the
    # metadata issuer otherwise. For a path-scoped advertisement whose metadata names the origin
    # (#116233) the two differ, and a binding the next browser connect reads as a *different* server
    # would drop these credentials (and the tokens) instead of reusing them.
