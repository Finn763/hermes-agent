...
def _persisted_identity() -> str:
    ...
    Environment first (the dispatcher pins ``HERMES_PROFILE``; some
    launchers set ``HERMES_PROFILE_NAME``), else the active profile from
    ``hermes_cli.profiles.get_active_profile_name`` — which resolves via
    ``get_hermes_home()``, so a multiplexed per-profile cron tick scoped
    with ``set_hermes_home_override`` attributes its own profile instead of
    the generic ``"worker"`` fallback (#119859). Never from tool args: board
    records are injected into future workers' prompts (see #19713).
    """
